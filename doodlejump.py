...
class doodle:
    ...
    def update(self):
        ...
        oldy = int(round(min(height(self.pos[1])//100, num_plat - 1)))  # ! changed by OPi to run also in Python (int require)
        newy = int(round(min(height(self.pos[1]+self.vel[1])//100,num_plat - 1)))  # ! changed by OPi to run also in Python (int require)
...
def draw(canvas):
    ...
    for steps in range(int(round(100*(offset[1]//100))),int(round(h+offset[1])),100):  # ! changed by OPi to run also in Python (int require)
